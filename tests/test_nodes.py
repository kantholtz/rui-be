...
from tests.fixtures.fixtures import deep_node_a, deep_node_c, deep_node_b, deep_node_ba, next_node_id, next_entity_id, \
    deep_node_aa
...
def test_get_nodes(client):
    ...
    assert sorted(nodes) == sorted([deep_node_a, deep_node_b, deep_node_c])
...
def test_post_root_node(client):
    ...
    assert sorted(nodes) == sorted([deep_node_a, deep_node_b, deep_node_c, expected_new_node])
...
def test_post_child_node(client):
    ...
    expected_node_a = deepcopy(deep_node_a)
    expected_node_a.children.append(expected_new_node)

    assert sorted(nodes) == sorted([expected_node_a, deep_node_b, deep_node_c])
...
def test_patch_root_node(client):
    ...
    node_patch = NodePatch(parent_id=deep_node_a.id)

    response = client.patch(f'http://localhost:5000/api/1.6.0/nodes/{deep_node_b.id}',
    ...
    expected_node_b = deepcopy(deep_node_b)
    expected_node_b.parent_id = deep_node_a.id

    expected_node_a = deepcopy(deep_node_a)
    expected_node_a.children.append(expected_node_b)

    assert sorted(nodes) == sorted([expected_node_a, deep_node_c])
...
def test_patch_child_node(client):
    ...
    child_node_patch = NodePatch(parent_id=deep_node_a.id)

    response = client.patch(f'http://localhost:5000/api/1.6.0/nodes/{deep_node_ba.id}',
    ...
    expected_node_a: DeepNode = deepcopy(deep_node_a)
    expected_node_b: DeepNode = deepcopy(deep_node_b)
    ...
    assert sorted(nodes) == sorted([expected_node_a, expected_node_b, deep_node_c])
...
def test_delete_root_node(client):
    ...
    response = client.delete(f'http://localhost:5000/api/1.6.0/nodes/{deep_node_a.id}')
    ...
    assert sorted(nodes) == sorted([deep_node_b, deep_node_c])
...
def test_delete_child_node(client):
    ...
    response = client.delete(f'http://localhost:5000/api/1.6.0/nodes/{deep_node_aa.id}')
