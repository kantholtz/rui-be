...
from rui_be.models.prediction.prediction_patch import (
    PredictionPatch,
    PredictionPatchSchema,
)
from rui_be.models.prediction.predictions_page import (
    PredictionsPage,
    PredictionsPageSchema,
)
...
def get_predictions(node_id: int) -> Response:
    ...
    candidate_to_homag_predictions: dict[
        str, list[draug.homag.model.Prediction]
    ...
    partial_predictions = [
        _build_partial_prediction(candidate, homag_predictions)
        for candidate, homag_predictions in candidate_to_homag_predictions.items()
    ]

    partial_predictions.sort(
        key=lambda it: (it.total_score_norm, it.total_score), reverse=True
    )
    ...
    predictions = [
        _build_prediction(partial_prediction)
    ...
    predictions_page = PredictionsPage(
        total_predictions=len(partial_predictions),
...
def _build_partial_prediction(
    candidate: str, homag_predictions: list[draug.homag.model.Prediction]
) -> PartialPrediction:
    ...
    total_score_norm, total_score = _calc_total_scores(
        synonym_predictions, parent_predictions
    )
...
def _build_partial_prediction_item(
    ...
    return PartialPredictionItem(
        score=homag_prediction.score,
...
def _calc_total_scores(
    ...
    if len(synonym_predictions) > 0 and len(parent_predictions) > 0:
        return (
            (synonym_predictions[0].score_norm + parent_predictions[0].score_norm) / 2,
...
def _build_prediction(partial_prediction: PartialPrediction) -> Prediction:
    parent_predictions = [
        _build_prediction_item(partial_prediction_item)
    ...
    return Prediction(
        candidate=partial_prediction.candidate,
...
def _build_prediction_item(
    partial_prediction_item: PartialPredictionItem,
) -> PredictionItem:
    ...
    return PredictionItem(
        score=partial_prediction_item.score,
...
def patch_prediction(candidate: str) -> tuple[str, int]:
    ...
    if (
        prediction_patch.dismissed
        and candidate in state.predictions_store.get_canidates()
    ):
