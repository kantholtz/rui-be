...
def get_nodes() -> Response:
    with ctx as state:
        ...
        def deep_node_from_nid(nid: int) -> DeepNode:
            ...
            predictions = state.predictions_store.count_by_nid(nid=nid)
            ...
            return DeepNode(
                ...
                childPredictions=predictions[graph.Graph.RELATIONS.parent],
                synonymPredictions=predictions[graph.Graph.RELATIONS.synonym],
            )
...
def post_node() -> tuple[str, int]:
    with ctx as state:
        ...
        nid = state.graph.add_node(
            entities=[graph.Entity(ent.name) for ent in req.entities]
        )
